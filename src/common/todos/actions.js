...
export function addHundredTodos() {
    ...
    return ({ getUid, now }) => {
        ...
        return {type: ADD_HUNDRED_TODOS, payload};
    };
}
...
export function clearAllCompletedTodos() {
    return { type: CLEAR_ALL_COMPLETED_TODOS };
}
...
export function fetchUserTodos() {
    return ({ fetch }) => ({
        ...
        payload: {
            promise: fetch('/api/v1/todos/user').then(response => response.json())
        }
    });
}
...
export function toggleTodoCompleted(todo) {
    return {
        type: TOGGLE_TODO_COMPLETED, payload: {
            todo
        }
    };
}
